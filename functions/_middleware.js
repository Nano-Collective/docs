 ...
 * emits (see scripts/generate-llms-content.ts):
 *
 *   1. RFC 8288 `Link` response headers on every page, pointing agents at
 *      /llms.txt (the machine-readable index of every doc), the sitemap, and
 *      the page's own Markdown mirror.
 *   2. Content negotiation for Markdown: a request that prefers
 *      `text/markdown` is served the page's pre-generated `.md` mirror — the
 *      raw Markdown source of the doc. Browsers, which never ask for Markdown,
 *      keep getting HTML.
 *
 * Only page routes run through here — public/_routes.json excludes the static
 * assets so images, /_next/* and the Pagefind index are served straight from
 * the CDN.
 */

/** Links advertised on every page, independent of the route. */
const SITE_LINKS = [
  '</llms.txt>; rel="service-doc"; type="text/plain"; title="Nano Collective documentation index"',
...
/**
 * Files that are served as themselves, never as a page: assets, RSC prefetch
 * payloads, and the Markdown mirrors. Matched by extension rather than by "the
 * last segment contains a dot", because version segments do — the docs index
 * for a release lives at /nanocoder/docs/v1.29.0/.
 */
const FILE_EXTENSION =
  /\.(md|txt|json|xml|html|js|mjs|css|map|png|jpe?g|gif|svg|webp|avif|ico|webmanifest|woff2?|ttf|mp4|webm|pdf|zip|wasm)$/i;

/**
 * The Markdown mirror for a page route. Routes are trailing-slashed
 * (next.config.mjs), and mirrors are written without the slash:
 * /collective/organisation/ -> /collective/organisation.md, / -> /index.md.
 */
