...
After the 30-day window, the core team opens a PR removing the sponsor's logo from every NC surface — same list of files as the onboarding PRs in reverse. The source files in `public/sponsors/` are kept in the repo (so historical pages and the Open Collective ledger remain consistent) but no longer referenced anywhere.
