  ...
  try {
    ...
    for (const project of projects) {
      ...
      if (versions.length > 0) {
        ...
        const latestPaths = await getDocPathsForVersion(
          project.id,
          latestVersion,
          project.repo,
        );
    ...
    console.log(
      `✓ Generated ${params.length} static params across all projects`,
    );
    return params;
  } catch (error) {
...
async function getFirstDocSlug(
  ...
  return firstFile.replace(/^docs\//, "").replace(/\.(md|mdx)$/, "");
}
...
export async function generateMetadata({
  ...
  try {
    const filePath = await findDocFile(
      projectId,
      resolvedVersion,
      slug,
      project.repo,
    ...
    const rawMdx = await fetchFileContent(
      resolvedVersion,
      filePath,
      project.repo,
    );
...
export default async function DocPage({ params }: PageProps) {
  ...
  if (!slug || slug.length === 0) {
    const indexFile = await findDocFile(
      projectId,
      resolvedVersion,
      undefined,
      project.repo,
